...
1. **Сначала загрузить навык `figma-use`** (resource `skill://figma/figma-use/SKILL.md`) — все привязки идут
   через `use_figma` (JS в Figma Plugin API). Без него — типовые трудноуловимые ошибки.
...
Привязываем только свойства, которыми владеет компонент; значения токенов = текущим (вид не меняется).
...
После аудита — `node.screenshot()` для визуального контроля (вид не должен измениться).
